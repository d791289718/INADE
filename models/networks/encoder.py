...
class ConvEncoder(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        self.so = s0 = 4
        self.fc_gamma_bias = nn.Linear(ndf * 8 * s0 * s0, 128)
        self.fc_gamma_scale = nn.Linear(ndf * 8 * s0 * s0, 128)
        self.fc_beta_bias = nn.Linear(ndf * 8 * s0 * s0, 128)
        self.fc_beta_scale = nn.Linear(ndf * 8 * s0 * s0, 128)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        gamma_bias = self.fc_gamma_bias(x)
        gamma_scale = self.fc_gamma_scale(x) 
        beta_bias = self.fc_beta_bias(x)
        beta_scale = self.fc_beta_scale(x)
        return gamma_scale, gamma_bias, beta_scale, beta_bias
