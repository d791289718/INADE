...
class GIDDataset(Pix2pixDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--split_dir', type=str, default='./datasets/GID/split_txt')
        return parser
    ...
    def get_paths(self, opt):
        ...
        with open(os.path.join(opt.split_dir, opt.phase + '.txt'), 'r') as f:
            for name in f:
                ...
                image_paths.append(os.path.join(opt.image_dir, name+'.tif'))
        ...
        file_name = os.path.join(os.path.join(txt_root, opt.name), 'dataset')
        with open(file_name + '.txt', 'wt') as opt_file:
            opt_file.write("dataset size: ")
